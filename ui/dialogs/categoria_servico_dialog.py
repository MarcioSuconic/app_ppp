...
class CategoriaServicoDialog(QDialog):
    def __init__(self, parent=None, categoria_id=None):
        ...
        self.nome_edit.setPlaceholderText("Ex: Manutenção, Costura, Aulas...")
    ...
    def carregar_dados(self):
        dados = CrudCategoriaServico.buscar_por_id(self.categoria_id)
        if dados:
    ...
    def aceitar(self):
        ...
        if self.categoria_id is None:
            sucesso, msg = CrudCategoriaServico.criar(nome, descricao)
            if sucesso:
                if not ativo:
                    categorias = CrudCategoriaServico.listar_todos(apenas_ativos=False)
                    for cat in categorias:
                        if cat["nome"] == nome:
                            CrudCategoriaServico.atualizar(cat["id"], nome, descricao, ativo)
                            break
        else:
            sucesso, msg = CrudCategoriaServico.atualizar(self.categoria_id, nome, descricao, ativo)
