...
class ServicoDialog(QDialog):
    def __init__(self, parent=None, servico_id=None):
        super().__init__(parent)
        self.servico_id = servico_id
        self.setWindowTitle("Novo Serviço" if servico_id is None else "Editar Serviço")
    ...
    def setup_principal_tab(self):
        ...
        layout.addRow(QLabel("Nome do Serviço:*"), self.nome_edit)
        ...
        self.duracao_spin.setRange(0, 1440)
        self.duracao_spin.setSuffix(" minutos")
        layout.addRow(QLabel("Duração Padrão:"), self.duracao_spin)
        ...
        layout.addRow(QLabel("Custo de Execução:"), self.custo_spin)
        
        self.ativo_check = QCheckBox("Serviço ativo")
    ...
    def setup_operacoes_tab(self):
        ...
        layout.addWidget(QLabel("Operações onde este serviço é oferecido:"))
    ...
    def setup_equipamentos_tab(self):
        ...
        layout.addWidget(QLabel("Equipamentos usados para executar este serviço:"))
    ...
    def carregar_categorias(self):
        ...
        for cat in CrudCategoriaServico.listar_todos(apenas_ativos=True):
    ...
    def carregar_equipamentos_vinculados(self):
        ...
        if self.servico_id:
            for eq in CrudServico.listar_equipamentos_vinculados(self.servico_id):
    ...
    def adicionar_equipamento(self):
        ...
        if ok and nome:
            ...
            tempo, ok = QInputDialog.getInt(self, "Tempo de Execução", "Tempo em minutos:", 1, 1, 1440)
            if ok:
                if self.servico_id:
                    sucesso, msg = CrudServico.vincular_equipamento(self.servico_id, equipamento["id"], tempo)
                    if sucesso:
    ...
    def remover_equipamento(self):
        ...
        if self.servico_id:
            sucesso, msg = CrudServico.desvincular_equipamento(self.servico_id, equipamento_id)
            if sucesso:
    ...
    def carregar_dados(self):
        dados = CrudServico.buscar_por_id(self.servico_id)
        if dados:
            self.nome_edit.setText(dados["nome"])
            self.descricao_edit.setText(dados["descricao"] or "")
            self.duracao_spin.setValue(dados["duracao_padrao_minutos"] or 0)
            ...
            operacoes_vinculadas = CrudServico.listar_operacoes_vinculadas(self.servico_id)
    ...
    def aceitar(self):
        ...
        if not nome:
            QMessageBox.warning(self, "Aviso", "O nome do serviço é obrigatório.")
            return
        ...
        if self.servico_id is None:
            sucesso, novo_id, msg = CrudServico.criar(nome, descricao, duracao, custo, preco, categoria_id)
            if sucesso:
                self.servico_id = novo_id
                for i in range(self.operacoes_list.count()):
                    item = self.operacoes_list.item(i)
                    if item.isSelected():
                        CrudServico.vincular_operacao(self.servico_id, item.data(Qt.ItemDataRole.UserRole))
                for i in range(self.equipamentos_list.count()):
                    item = self.equipamentos_list.item(i)
                    equipamento_id = item.data(Qt.ItemDataRole.UserRole)
                    tempo = item.data(Qt.ItemDataRole.UserRole + 1)
                    if equipamento_id:
                        CrudServico.vincular_equipamento(self.servico_id, equipamento_id, tempo)
            else:
                QMessageBox.critical(self, "Erro", msg)
                return
        else:
            sucesso, msg = CrudServico.atualizar(self.servico_id, nome, descricao, duracao,
                                                custo, preco, categoria_id, ativo)
            if not sucesso:
                QMessageBox.critical(self, "Erro", msg)
                return
            
            operacoes_atuais = [op["id"] for op in CrudServico.listar_operacoes_vinculadas(self.servico_id)]
            novas_operacoes = []
            for i in range(self.operacoes_list.count()):
                item = self.operacoes_list.item(i)
                if item.isSelected():
            ...
            for op_id in operacoes_atuais:
                if op_id not in novas_operacoes:
                    CrudServico.desvincular_operacao(self.servico_id, op_id)
            for op_id in novas_operacoes:
                if op_id not in operacoes_atuais:
                    CrudServico.vincular_operacao(self.servico_id, op_id)
